...
仮説を主張に分け（原文と並べて見せます）、有報の対象節を段落に切り、段落ごとに主張を当てます。数値の主張はコードが財務データで検算し、「一致 / 不一致 / 検算不能」で返します。段落から確かめられなければ「判定不能」で止めます。モデルに埋めさせません。
...
- `/check` と `/watch` は、売買の指示・目標株価・建玉の大きさを出力しません
...
`bun run demo` が再生する段落は、有価証券報告書の本文です。1 件ずつに書類管理番号・提出者・書類種別・箇所・取得日・出典表記・編集加工の主体を持たせています。
