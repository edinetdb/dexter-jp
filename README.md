...
会話履歴の埋め込みは既定で有効で、選択中の LLM とは独立に送信先が決まります。`/model` で Claude を選んでいても、`.env` に `OPENAI_API_KEY` があれば会話は OpenAI に送られます。起動時に、`.env` に設定された鍵から、そのセッションで外へ出うる先の一覧が出ます（一覧への掲載は実際の送信を意味しません。送信の条件は機能ごとに異なり、会話履歴の埋め込み先は自動で選ばれます）。
