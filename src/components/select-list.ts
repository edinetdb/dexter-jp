...
export function createApprovalSelector(
  ...
  if (proposedRule) {
    ...
    items.push({
      value: 'allow-session',
      label: queryScoped
        ? '3. Yes, allow this exact operation this query'
        : '3. Yes, allow this exact operation this session',
    });
    items.push({ value: 'deny', label: '4. No' });
  } else {
    items.push({ value: 'allow-session', label: '2. Yes, allow this exact operation this session' });
